...
def check_all_models():
    ...
    # This function checks all possible combinations of Truth values, and parses through them to check if one
    # of the combinations returns true for kb_eval and false for infer_eval. If so, it returns False
    model = {}
    allpossible = list(itertools.product([True, False], repeat=len(identifiers)))
    for i, combination in enumerate(allpossible):
        combination_dict = {identifier: value for identifier, value in zip(identifiers, combination)}
        model[f'combination_{i + 1}'] = combination_dict
    for combination in model:
        kb_eval = evaluate_expression_set(kb, model[combination])
        infer_eval = evaluate_expression_set(infer, model[combination])
        if kb_eval and not infer_eval:
            print("kb does not entail infer for", model[combination])
            return False
